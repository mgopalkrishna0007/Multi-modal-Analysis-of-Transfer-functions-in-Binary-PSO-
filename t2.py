...
# v6 compaired with variant 1 and 2
...
def v6_transfer_function(v, variant_type, iteration=None, max_iter=None, velocity_history=None):
    ...
    """
    if variant_type == 1:
        # Type 1 (Baseline): Constant n=2
        return 1 - np.exp((-1/2)*np.abs(v)**2)
    elif variant_type == 2:
        # Type 2: Linearly decreasing n from 7 to 1
        n = 7 - 6 * (iteration / max_iter)
        return 1 - np.exp((-1/2)*np.abs(v)**n)
    elif variant_type == 3:
        # Type 3: Piecewise n=7 for first half, n=2 for second half
        n = 7 if iteration < max_iter/2 else 2
        return 1 - np.exp((-1/2)*np.abs(v)**n)
    elif variant_type == 4:
        # Type 4: Velocity-based n (7 if |v|>1, else 1)
        n = np.where(np.abs(v) > 1, 7, 1)
        return 1 - np.exp((-1/2)*np.abs(v)**n)
    else:
